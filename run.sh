...
if [[ "${1:-}" == "--model" ]]; then
    ...
    MOUNT_DIR="$(pwd)/hermes-data"
    if command -v cygpath &>/dev/null; then
        export MSYS_NO_PATHCONV=1
        MOUNT_DIR="$(cygpath -w "$MOUNT_DIR")"
    elif command -v wslpath &>/dev/null; then
        MOUNT_DIR="$(wslpath -w "$MOUNT_DIR")"
    fi
    exec docker run --rm -it -v "${MOUNT_DIR}:/root/.hermes" hermes-agent:local hermes model
    exit 0
fi
...
# ── Prepare Docker Mount Path ──────────────────────────────────────────────────
DOCKER_MOUNT_DIR="${HERMES_DATA_DIR}"
if command -v cygpath &>/dev/null; then
    export MSYS_NO_PATHCONV=1
    DOCKER_MOUNT_DIR="$(cygpath -w "${HERMES_DATA_DIR}")"
elif command -v wslpath &>/dev/null; then
    DOCKER_MOUNT_DIR="$(wslpath -w "${HERMES_DATA_DIR}")"
fi
...
DOCKER_ARGS=(
    ...
    -v "${DOCKER_MOUNT_DIR}:/root/.hermes"
...
if [[ -f "${HERMES_DATA_DIR}/.env" ]] && [[ ! -w "${HERMES_DATA_DIR}/.env" ]]; then
    ...
    docker run --rm -v "${DOCKER_MOUNT_DIR}:/data" "${DOCKER_IMAGE}" chown $(id -u):$(id -g) /data/.env 2>/dev/null || true
fi
